...
function estUnNom($valeur)
{
   return preg_match("`[A-Za-z_\-\.]`",$valeur);
}
function estUneVille($valeur)
{
   return preg_match("`[A-Za-z_\-\.]`",$valeur);
}
// Si la valeur transmise ne contient pas d'autres caractères que des chiffres  
...
function verifierDonneesEtabM($connexion, $id, $nom, $adresseRue, $codePostal, 
                              $ville, $tel, $adresseElectronique, $nomResponsable, $prenomResponsable, $nombreChambresOffertes)
{
   ...
   if($nom != "" && !estUnNom($nom))
   {
	ajouterErreur("Le nom doit être une chaine de caractère");
	}
   if ($codePostal!="" && !estUnCp($codePostal))
   {
      ajouterErreur("Le code postal doit comporter 5 chiffres");   
   }
   if ($ville!="" && !estUneVille($ville))
   {
      ajouterErreur("La ville doit être une chaîne de caractères");   
   }
   if ($tel !="" && !estEntier($tel))
   {
      ajouterErreur("Le téléphone doit être constitué de chiffres");
   }
   ...
   {
	ajouterErreur("Le téléphone doit comporter 10 chiffres");
	}
    if (preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $adresseElectronique))
    {
    }
    else
    {
	...
	if($nomResponsable != "" && !estUnNom($nomResponsable))
   {
	ajouterErreur("Le nom  du responsable doit être une chaine de caractère");
	}
	 if($prenomResponsable != "" && !estUnNom($prenomResponsable))
   {
	ajouterErreur("Le prénom du responsable doit être une chaine de caractère");
	}
